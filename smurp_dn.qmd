...
alldata_dn <- alldata_ar |> 
  mutate(dn_rate_max = pmap_dbl(list(data_norm, t_peak - 1000),
                                calc_rate),
...
alldata |> 
  select(experiment, replicate,
         t_anox, t_peak,
         ar_loss_rate, dn_rate_o2_min,
         dn_rate_o2_min_ar_cor,
         dn_rate_max) |> 
  mutate(t_anox = t_anox/3600,
         t_peak = t_peak/3600) |> 
knitr::kable(digits = c(1, 1, 1, 1, 0, 0, 0, 0),
             col.names = c("Experiment", "Replicate",
                           "Time to Anoxia [h]", "Time to Peak [h]",
                           "Ar loss [umol/d]", "DN Rate (O2 min) [umol/d]",
                           "DN Rate (O2 min Ar cor)", "DN Rate (max) [umol/d]"))
```

# Summary plots

```{r}
init <- ggplot(alldata, aes(experiment, dn_rate_o2_min_ar_cor)) +
  geom_boxplot() +
...
ar <- ggplot(alldata, aes(experiment, -ar_loss_rate)) +
  geom_boxplot() +
  ylab("Ar loss rate [umol/day]")

ox <- ggplot(alldata, aes(experiment, t_anox)) +
  geom_boxplot() +
...
```{r}
a <- aov(dn_rate_o2_min_ar_cor ~ experiment, data = alldata)
summary(a)
```

```{r}
a <- aov(dn_rate_o2_min_ar_cor ~ grass + open, data = alldata)
summary(a)
```
...
```{r}
a <- aov(dn_rate_o2_min_ar_cor ~ grass, data = alldata)
summary(a)
```
...
```{r}
a <- aov(dn_rate_o2_min_ar_cor ~ replicate, data = alldata)
summary(a)
```
